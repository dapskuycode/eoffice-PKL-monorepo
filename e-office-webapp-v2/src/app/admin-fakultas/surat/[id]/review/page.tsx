...
export default function AdminFakultasReviewSurat() {
  ...
  // Helper to ensure user is available with fallback to localStorage
  const ensureUser = () => {
  ...
  const handleDaftarkan = async () => {
    ...
    try {
      ...
      if (success) {
        setPengajuan({ ...pengajuan, status: 'REGISTERED' });
    ...
    <div className="h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex" style={{ marginTop: '64px' }}>
        ...
        <aside className="w-64 bg-white shadow-md fixed left-0 h-[calc(100vh-64px)] overflow-y-auto" style={{ top: '64px' }}>
          <SidebarMenu role="admin_fakultas" collapsed={false} />
        </aside>
                ...
                {pengajuan.status === 'COMPLETED' ? (
                  ...
                  <div className="flex gap-3 mt-6 justify-end">
                    ...
                    <Button
                      ...
                      disabled={submitting}
                    >
                      <CheckCircleOutlined /> {submitting ? 'Memproses...' : 'Daftarkan'}
                    </Button>
                  </div>
                ) : (
