...
export default function StafFakultasReviewSurat() {
  ...
  const fetchPengajuanDetail = async () => {
    ...
    try {
      setLoading(true);
      const pengajuanData = await staffFakultasService.getPengajuanDetail(params.id as string);
  ...
  const handleKerjakan = async () => {
    ...
    try {
      ...
      if (success) {
        alert('Surat telah dicetak dan dikirim ke UPA!');
      ...
      <div className="flex" style={{ marginTop: '64px' }}>
        ...
        <main className="flex-1 p-8 pt-8" style={{ marginLeft: '256px' }}>
          ...
          <div className="mb-6 text-sm text-gray-600 pt-2">
            <span className="cursor-pointer hover:text-blue-600" onClick={() => router.push('/staff-fakultas/dashboard')}>Surat Masuk</span>
