...
export default function UpaDashboard() {
  ...
  const getStatusColor = (status: string) => {
    const colors: { [key: string]: string } = {
      ...
      COMPLETED: 'bg-emerald-100 text-emerald-800', // Selesai untuk UPA
  ...
  const formatStatus = (status: string) => {
    // Tampilkan COMPLETED sebagai "Selesai" untuk UPA
    if (status === 'COMPLETED') {
      return 'Selesai';
    }
  ...
  const currentItems = filteredPengajuan
    .filter(item => item.status === 'STEP_KONVENSIONAL') // Filter for Dashboard
    .slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(filteredPengajuan.filter(item => item.status === 'STEP_KONVENSIONAL').length / itemsPerPage);

  return (
    <UPALayout>
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Finalisasi Surat - UPA</h1>
        <p className="text-gray-600">Finalisasi dan penerbitan nomor SKL resmi</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-gray-600">Perlu Finalisasi</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-green-600">
              {pengajuan.filter(p => p.status === 'COMPLETED').length}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Daftar Pengajuan SKL (Perlu Finalisasi)</CardTitle>
