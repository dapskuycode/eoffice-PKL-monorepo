...
const PENDING_STATUSES = [
  ...
  "DISETUJUI_SUPERVISOR_AKADEMIK",
  "PENOMORAN",
];

const FormSuratKeteranganMahasiswa: React.FC = () => {
  ...
  const mapExistingApplicationToForm = (
    ...
    try {
      ...
      const formData: DetailData = {
        ...
        keperluan: parsedInfo.keperluan || "",
      };
  ...
  const handleSubmitForm = async (values: Record<string, any>) => {
    try {
      ...
      // console.log("📎 Current listLampiran:", listLampiran);
      // console.log("📄 Current fileKTM:", fileKTM);
      if (values.tempat_lahir && values.tanggal_lahir) {
        // Konversi menggunakan dayjs
        const tanggalLahir = dayjs(values.tanggal_lahir);

        // Format tanggal dalam bahasa Indonesia
        const hari = tanggalLahir.date();
        const bulan = tanggalLahir.format("MMMM"); // Pastikan dayjs sudah pakai locale 'id'
        const tahun = tanggalLahir.year();

        values.ttl = `${values.tempat_lahir}, ${hari} ${bulan} ${tahun}`;

        // Hapus field sementara
        delete values.tempat_lahir;
        delete values.tanggal_lahir;
      }
      console.log(values);
      ...
      formData.append("tipe_suratId", "ak007");
      ...
      if (isRevisionNeeded && existingApplication?.id) {
        ...
        try {
          ...
          if (
            ...
            handleNext();
            message.success(
              "Pengajuan Surat Keterangan Mahasiswa Berhasil Dikirim!",
            );
          } else {
            console.error(
                  ...
                  <div>
                    ...
                    >
                      ...
                      <div style={{ margin: "12px 0 0 0" }}>
                        Berikut adalah panduan dan ketentuan dalam pengajuan
                        Surat Keterangan Mahasiswa:
                      </div>
                      <ol style={{ margin: "12px 0 0 18px", padding: 0 }}>
                        <li>
                          <b>Isi formulir dengan lengkap dan benar.</b> Pastikan
                          seluruh data mahasiswa dan data orang tua terisi
                ...
                <Row gutter={16}>
                  <Col span={24} lg={12} md={12} sm={24}>
                    <ProFormText
                      ...
                      rules={[
                        { required: true, message: "Mohon diisi" },
                        { min: 2, message: "Nama tempat terlalu pendek" },
                      ]}
                ...
                <Row gutter={16}>
                  <Col span={24} lg={12} md={12} sm={24}>
                    <ProFormText
                      ...
                      name="alamat"
                      rules={[{ required: true, message: "Mohon diisi" }]}
                      placeholder="Contoh: Jl. Prof. Soedarto, Tembalang, Kec. Tembalang, Kota Semarang, Jawa Tengah 50275"
                    />
                  </Col>
                </Row>
              </Card>
              ...
              <div style={{ minHeight: "200px" }}>
                ...
                >
                  <div style={{ textAlign: "center", marginBottom: "20px" }}>
                    ...
                    <div style={{ color: "#666" }}>
                      Pengajuan Surat Keterangan Mahasiswa Anda sedang diproses
                      fakultas
                    </div>
                  </div>
                </Space>
...
export default FormSuratKeteranganMahasiswa;
