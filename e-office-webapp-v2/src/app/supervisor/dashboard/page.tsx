...
function getStatusDisplay(status: string) {
  ...
  const statusMap: Record<string, { label: string; color: string }> = {
    ...
    // Semua status setelah APPROVED_SUPERVISOR ditampilkan sebagai "Selesai" untuk Supervisor
...
export default function SupervisorDashboard() {
    ...
    try {
      ...
      if (data) {
        const formattedLetters = data.pengajuan
          .filter((pengajuan: any) => pengajuan.status === 'REGISTERED') // Filter for Dashboard
          .map((pengajuan: any) => {
            ...
            const namaDisplay = pengajuan.namaSementara || mahasiswa?.user?.name || 'Unknown';
  ...
  return (
    <SupervisorLayout>
      ...
      {/* Table Section */}
      <Card className="border-none overflow-hidden bg-white/50 backdrop-blur-xl">
        <CardHeader className="border-b border-gray-100/50 pb-6 px-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <CardTitle className="text-xl font-bold text-gray-800">Surat Perlu Tindakan</CardTitle>
              <p className="text-xs text-gray-500 mt-1 font-medium">Daftar pengajuan yang menunggu keputusan Anda</p>
            </div>
            <div className="flex items-center gap-3">
              <div className="relative group">
                <input
                  ...
                  className="pl-11 pr-4 py-2.5 bg-gray-50 border-none rounded-2xl text-sm w-full md:w-64 focus:ring-2 focus:ring-blue-500/20 transition-all group-hover:bg-gray-100"
                  value={searchTerm}
