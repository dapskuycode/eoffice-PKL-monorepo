...
function getStatusDisplay(status: string) {
  const statusMap: Record<string, { label: string; color: string }> = {
    ...
    // Semua status setelah REGISTERING ditampilkan sebagai "Selesai" untuk Admin Prodi
...
export default function AdminProdiDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      if (dashboardData) {
        // Format pengajuan for table display
                  ...
                  <tr key={letter.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <button
                        ...
                        onClick={() => {
                          // If status is APPROVED_KAPRODI, go to generate page (for numbering)
                          // Otherwise, go to detail page
                          if (letter.rawStatus === 'APPROVED_KAPRODI') {
                            router.push(`/admin-prodi/surat/${letter.id}/generate`);
                          } else {
                            router.push(`/admin-prodi/surat/${letter.id}`);
                          }
                        }}
                      >
