...
function SuratKeluarVerification() {
          ...
          try {
            if (status === "DISETUJUI") {
              ...
              if (access.isPetugasAka) {
                ...
                if (access.isSpvAka) {
                  roleTujuan = ["ak006", "ak007"].includes(
                    suratKeluar?.surat_masuk?.tipe_suratId ?? "",
                  )
                    ? 1
                    : 9;
                } else if (access.isSpvSda) {
                  roleTujuan = 1; // SPV Sumberdaya menolak ke Mahasiswa
                } else if (access.isKtu) {
                  // MTU menolak - cek jenis surat
                  roleTujuan = ["ak006", "ak007"].includes(
                    suratKeluar?.surat_masuk?.tipe_suratId ?? "",
                  )
                    ? 1
                  ...
                  roleTujuan = 8; // WD1 menolak ke MTU
                }
                await axios.patch(
                  `/v1/pengajuan/surat-keluar/${id}/changeStatus`,
                  {
  ...
  return (
    <div className="p-6">
      ...
      <Card>
        <EnhancedSteps
          currentStep={getCurrentStep(suratKeluar?.surat_masuk?.tipe_suratId, {
          ...
          >
            <Card title="Tindakan Supervisor" style={{ marginTop: 16 }}>
              ...
              >
                ...
                {/* Upload Section - CONDITIONAL RENDERING DENGAN TERNARY */}
                {suratKeluar?.surat_masuk?.tipe_suratId === "ak15" && (
                  <div
                    style={{
                      padding: "16px",
                      background: "#fff",
                ...
                <Row justify="space-between" align="middle">
                  ...
                  <Col>
                    <ResponsiveSpace size="middle">
                      <Button icon={<EyeOutlined />} onClick={handlePreview}>
                      Preview Data {(() => {
                        const tipe = suratKeluar?.surat_masuk?.tipe_suratId;
                        if (tipe === "srb") return "Surat Rekomendasi Beasiswa";
                        if (tipe === "ak006") return "Surat Keterangan Mahasiswa untuk Tunjangan PNS/BUMN";
                        if (tipe === "ak007") return "Surat Keterangan Mahasiswa";
                        if (tipe === "ak15") return "Surat Pengantar";
                        return "Surat";
                      })()}
                      </Button>
                      <Button icon={<EyeOutlined />} onClick={handlePreviewPDF}>
