...
import KetuaProdiLayout from '@/components/layout/KetuaProdiLayout';

function getStatusDisplay(status: string) {
    const statusMap: Record<string, { label: string; color: string }> = {
        'DRAFT': { label: 'Draft', color: 'text-gray-600' },
        'SUBMITTED': { label: 'Diajukan', color: 'text-yellow-600' },
        'VERIFIED_ADMIN': { label: 'Belum Ditandatangani', color: 'text-yellow-600' },
        'APPROVED_KAPRODI': { label: 'Selesai', color: 'text-green-700' },
...
export default function KetuaProdiSemuaSurat() {
    ...
    const fetchDashboardData = async () => {
        try {
            setLoading(true);
            const dashboardData = await kaprodiService.getDashboard();

            if (dashboardData) {
                const formattedLetters = dashboardData.pengajuan.map((pengajuan: any) => {
    ...
    return (
        <KetuaProdiLayout>
            <div className="mb-8">
                ...
                <p className="text-gray-600">Daftar seluruh surat yang pernah masuk dan diproses oleh Ketua Program Studi.</p>
            </div>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            ...
                            >
                                ...
                                <option value="VERIFIED_ADMIN">Belum Ditandatangani</option>
                                <option value="APPROVED_KAPRODI">Selesai</option>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            ...
                            <tbody>
                                {loading ? (
                                    <tr>
                                        <td colSpan={8} className="py-10 text-center text-gray-500">Memuat data...</td>
                                    </tr>
                                ) : currentLetters.length === 0 ? (
                                    <tr>
                                        <td colSpan={8} className="py-10 text-center text-gray-500">Tidak ada surat</td>
                                    </tr>
                                ) : (
                                    currentLetters.map((letter) => (
                                        <tr key={letter.id} className="border-b hover:bg-gray-50">
                                            ...
                                            <td className="py-3 px-4 text-sm text-gray-800">{letter.pengusul}</td>
                                            ...
                                            <td className="py-3 px-4">
                                                <button
                                                    ...
                                                    onClick={() => router.push(`/ketua-prodi/surat/${letter.id}`)}
                                                >
