...
export default function ManajerTuReviewSurat() {
  ...
  const fetchPengajuanDetail = async () => {
    ...
    try {
      setLoading(true);
      const pengajuanData = await manajerTuService.getPengajuanDetail(params.id as string);
  ...
  const handleKirimKeStaf = async () => {
    ...
    try {
      if (!params || !params.id) return;
      const success = await manajerTuService.kirimKeStaf(
        params.id as string,
        actorId,
        'Dikirim ke Staf Fakultas untuk pencetakan'
      );

      if (success) {
        alert('Surat berhasil dikirim ke Staf Fakultas!');
        router.push('/manajer-tu/dashboard');
      } else {
  ...
  const namaLengkap = mahasiswa?.user?.name || 'Unknown';
  const nim = mahasiswa?.nim || '-';
  const tempatLahir = mahasiswa?.tempatLahir || '-';
  ...
  const noHp = mahasiswa?.noHp || '-';
  const programStudi = mahasiswa?.programStudi?.nama || '-';
      ...
      <div className="flex" style={{ marginTop: '64px' }}>
        ...
        <main className="flex-1 p-8 pt-8" style={{ marginLeft: '256px' }}>
          ...
          <div className="mb-6 text-sm text-gray-600 pt-2">
            <span className="cursor-pointer hover:text-blue-600" onClick={() => router.push('/manajer-tu/dashboard')}>Surat Masuk</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
                      ...
                      <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-500">{getCleanFileName(lamp.pathFile)}</p>
                          </div>
                        </div>
                        <button
              ...
              <div className="space-y-3">
                {pengajuan.status === 'APPROVED_SUPERVISOR' && (
                  <Button
                    className="w-full bg-green-500 hover:bg-green-600 text-white"
                    onClick={handleKirimKeStaf}
                  >
                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    Kirim ke Staf Fakultas
                  </Button>
                )}
